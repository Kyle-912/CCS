    ...
    while (1)
    {
        if (playing)
        {
            for (int col = 0; col < 8; col++)
            {
                ...
                // Highlight the current column with yellow (offset by one to the right)
                ST7789_DrawLine((col + 1) * cell_width, 0, (col + 1) * cell_width, Y_MAX, ST7789_YELLOW);         // Left vertical line
                ST7789_DrawLine((col + 2) * cell_width - 1, 0, (col + 2) * cell_width - 1, Y_MAX, ST7789_YELLOW); // Right vertical line

                // Clear the previous column highlight (if applicable)
                if (prev_col != -1 && prev_col != col)
                {
                    ST7789_DrawLine((prev_col + 1) * cell_width, 0, (prev_col + 1) * cell_width, Y_MAX, ST7789_WHITE);         // Left vertical line
                    ST7789_DrawLine((prev_col + 2) * cell_width - 1, 0, (prev_col + 2) * cell_width - 1, Y_MAX, ST7789_WHITE); // Right vertical line
                }
