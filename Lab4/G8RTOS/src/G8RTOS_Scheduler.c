...
void SysTick_Handler()
{
    ...
    for (int i = 0; i < NumberOfPThreads; i++)
    {
        ...
        if (Ppt->executeTime <= SystemTime) // TODO: Consider using current time instead of 
        {
...
sched_ErrCode_t G8RTOS_Add_PeriodicEvent(void (*PThreadToAdd)(void), uint32_t period, uint32_t execution)
{
    ...
    // TODO: consider setting current time based on number of pthreads and use current time instead of system time in SysTick_Handler
