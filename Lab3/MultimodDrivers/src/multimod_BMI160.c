...
void BMI160_Init()
{
    ...
    // Enable the auxiliary I2C interface for communication with BMM150
    ...
    // Power up the BMM150 magnetometer via auxiliary I2C interface
    // Write to the BMM150’s power control register (0x4B)
    BMI160_WriteRegister(BMI160_MAGCONF_ADDR, 0x01); // Enable normal mode for BMM150

    // Manually trigger auxiliary read from BMM150
    // Write to the BMI160 auxiliary control register to trigger data transfer
    BMI160_WriteRegister(BMI160_CMD_ADDR, 0x37); // Trigger auxiliary sensor read
