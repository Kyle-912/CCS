...
#define ADD_Qn_Qn(x, y, xn, yn) \
    ((xn) > (yn) ? (x) + ((y) << ((xn) - (yn))) : (x) + ROUND_OFF_Qn((y), ((xn) - (yn))))
...
int32_t goertzel(double detect_hz, double sample_rate_hz, int N, int16_t (*ptrSamplingFunc)(int), int FIFO_index)
{
    ...
    /* This complex feedforward coefficient allows a single zero to
       ...
       advance. */
    const double complex dbl_coeff_ff = -cexp(-I * 2 * 3.14159265358979323846 * m / N);
    ...
    double dbl_coeff_fb = 2 * cos(2 * 3.14159265358979323846 * m / N);
    ...
    while (1)
    {
        ...
        if (sample_index++ == N)
        {
            /* End of Goertzel algorithm for this buffer. Apply the
